...
public class BackLogService {
	...
	@Autowired
	private SprintRepository sprintRepositroy;
	
	public List<Issue>getBackLog(Long projectId){
		if(projectId==null) {
			return issueRepository.findBySprintIdOrderByBackLogPosition(projectId);
		}
		return issueRepository.findBySprintIdOrderByBackLogPosition(projectId);
	}
	...
	public Issue addIssueToSprint(Long issueId,Long sprintId) {
		Issue issue = issueRepository.findById(sprintId).orElseThrow(()-> new RuntimeException("Issue not found"));
		Sprint sprint = sprintRepositroy.findById(sprintId).orElseThrow(()-> new RuntimeException("Sprint not found"));
