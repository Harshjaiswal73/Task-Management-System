...
import com.example.Task.Management.Client.IssueClient;
...
public class IntegrationService {
	...
	public void handleCommitMessage(String commitMsg,String author) {
		String regex = "([A-Z]+-\\d+)";
		var matcher = java.util.regex.Pattern.compile(regex).matcher(commitMsg);
		...
		if(matcher.find()) {
			...
			issueclient.status(issueId,5, author);
	...
	public void handlePullRequest(String title,String author) {
		...
		if(matcher.find()) {
			...
			issueclient.status(issueId, status, issueKey);
	...
	public void processGithubEvent(String event,Map<String,Object>payload) {
		...
		if(event.equals("PUSH")) {
			...
			for(Object obj : commits) {
				...
				extractIssueKeyandMoveInprogress(msg,author);
			}
		}
	...
	public void processJenkinsEvent(Map<String,Object> body) {
		...
		if(m.find()) {
			...
			if(result.equals("FAILURE")) {
				issueclient.addCommit(issueId,"Jenkins","Build Failed\n''''\n"+log+"\n''''");
			}
		}
	...
	public void processDockerEvent(Map<String, Object>payload) {
		...
		if(issueKey == null) {
			System.out.println("No issue key found in docker");
			return;
		}
